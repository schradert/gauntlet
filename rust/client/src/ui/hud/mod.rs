...
fn open_wayland() -> Task<AppMsg> {
    ...
    let settings = iced_layershell::reexport::NewLayerShellSettings {
    ...
    Task::done(AppMsg::WaylandOpenWindow(id, settings))
        .then(move |_| in_2_seconds(AppMsg::WaylandCloseWindow(id)))
}
...
pub fn close_hud_window(
    ...
    #[cfg(target_os = "linux")]
    if wayland {
        Task::done(AppMsg::WaylandCloseWindow(id))
    } else {
        window::close(id)
    }
